...
## How it works?
1. Web user clicks payment button from Web app
2. Bank Server will generates a QR-code embeded with encrypted transaction information for web users to scan
3. After registered in Bank app, users are allowed to scan QR code with Bank app and approve or reject the transaction.
4. User approves transaction requirement and send back a sigital sign.
5. Bank server side can verify the approval by the RSA signature. This transaction will only succesed when the requirement is approved on bank app and verified the approval is made by the Bank account owner.
